...
def get_Redshift_connection():
    ...
    redshift_user = "jieunshop200"  # 본인 ID 사용
    redshift_pass = "Jieunshop200!1"  # 본인 Password 사용
    port = 5439
    dbname = "dev"
...
def load(lines):
    ...
    sql = "BEGIN;DELETE FROM jieunshop200.name_gender;"
    for l in lines:
        if l != '':
            (name, gender) = l.split(",")
            sql += "INSERT INTO jieunshop200.name_gender VALUES ('{name}', '{gender}');".format(name=name, gender=gender)
    sql += "END;"
    logging.info(sql)
    """
